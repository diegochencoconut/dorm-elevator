...
direction_mode dir_mode = FORWARD;
//===============================for debug
bool debug = false;
//===========================================
...
void adjust() {
  ...
  if (cnt == 0)
  {
    ...
    if (cnt <= 3) {
      ...
      else if(cur_mode == ENTERING)
      {
        cur_mode=STOPPED;//change
      }
    }
  }
}
...
void loop() {
  ...
  {
    case STARTED:
      if(dir_mode==FORWARD)blind_walking();
      else back_blind_walking();
      break;
    case STOPPED:
      MotorWrite(0.0, 0.0);
      break;
